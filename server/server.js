...
app.get('/admitad/statistics/actions/', (req, res) => {
  ...
  const results = pageData.map(tx => ({
    ...
    order_sum:         String(parseFloat(tx.order_sum  || 0).toFixed(2)),
    status:            tx.status           || 'pending',
    full_payment:      String(parseFloat(tx.payment || 0).toFixed(2)),
    payment_webmaster: String(parseFloat(tx.payment || 0).toFixed(2)),
    currency:          tx.currency         || 'RUB',
    website_id:        parseInt(tx.website_id) || 0,
